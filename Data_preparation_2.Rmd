...
```{r}
# ethnicity <- readr::read_csv("data/subset_gender.csv") %>% 
#   select(au_id, born_in, white_or_not) %>% 
#   rename(Country_origin = born_in,
#          Ethnicity_vis = white_or_not)
# 
# check <- df_ego %>% left_join(ethnicity, by = "au_id")

```
```{r}
# check %>% count(Country_origin)
```
